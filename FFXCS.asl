...
startup
{
    ...
    vars.bossRS = new int[,]{       {12000, 1420, 221, 1480, 2}, // Spherimorph
                                    ...
                                    {40000, 2585, 311, 2680, 0}  // Sanctuary Keeper
                                    };
    vars.boss_fight = -1;
}
...
update
{
    ...
    //game.WriteValue(modules.First().BaseAddress+0xD2CA9C, 2); // Spawn Point
    ...
    {
        print("Backflip skip + Tromell sphere smash");
    ...
    if (current.roomNumber == 306 && current.storyline == 2135/* && current.gameState == 1*/)
    {
        print("Trials to Bahamut Name");
        game.WriteValue(modules.First().BaseAddress+0xD2CA90, 226); //AreaID
        game.WriteValue(modules.First().BaseAddress+0xD2D67C, 2150); //CutsceneID
    }

    if (current.roomNumber == 226 && current.storyline == 2155 && current.menu == 0)
    {
        print("Bahamut to Via Purifico");
        game.WriteValue(modules.First().BaseAddress+0xD2CA90, 198); //AreaID
        game.WriteValue(modules.First().BaseAddress+0xD2D67C, 2220); //CutsceneID
        game.WriteValue(modules.First().BaseAddress+0xD2CA9C, 0); //Spawn
        game.WriteValue(modules.First().BaseAddress+0xF3080C, 1); //Force load

        // Fixing party formation to solo Yuna
        game.WriteBytes(modules.First().BaseAddress+0xD307E8, new byte[]{0x1, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF});

        // Removing party members from menu:
        game.WriteValue(modules.First().BaseAddress+0xD32088, (byte)0);  // Tidus
        game.WriteValue(modules.First().BaseAddress+0xD3211C, (byte)11); // Yuna
        game.WriteValue(modules.First().BaseAddress+0xD321B0, (byte)0);  // Auron
        game.WriteValue(modules.First().BaseAddress+0xD32244, (byte)0);  // Kimmy
        game.WriteValue(modules.First().BaseAddress+0xD322D8, (byte)0);  // Wakka
        game.WriteValue(modules.First().BaseAddress+0xD3236C, (byte)0);  // Lulu
        ...
        // Fix teleport platform to north
        game.WriteValue(modules.First().BaseAddress+0xD2CC84, 1); 
    } 
    ...
    if (current.roomNumber == 279 && current.storyline == 2420 &&
        current.xCoords > 250.0f && current.movementLock == 48)
    {
        //Defender X death
    ...
    if (current.roomNumber == 132 && current.storyline == 2680 && current.gameState == 0)
    {
    ...
    if (current.roomNumber == 202 && current.storyline == 3125 && 
        current.xCoords == 18.58586121f && current.gameState == 0)
    {
        print("Pre-OD Sin Yuna monologue");
        // TODO: Same as Airship -> Sin. This cutscene should spawn you in the corridor.
